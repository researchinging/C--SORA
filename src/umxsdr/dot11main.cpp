...
void StartBaseband11a () {
	...
	// Start all threads
	hTxThread = StartThread ( Dot11aSendProc, NULL);
	if (hTxThread == NULL ) {
	...
	hViterbiThread = StartThread ( BB11ARxViterbiWorker, &RxContext );
	if (hViterbiThread == NULL ) {
		printf("failed to start viterbi thread\n");
		return;
	}

	hRxThread = StartThread ( Dot11aRecvProc, &RxContext );
	if (hRxThread == NULL) {
...
void StartBaseband11b () {
	...
	// Start all threads
	hTxThread = StartThread ( Dot11bSendProc, NULL);
	if (hTxThread == NULL ) {
		printf("failed to start tx thread\n");
		return;
	}

	hRxThread = StartThread ( Dot11bRecvProc, &RxContext );
	if (hTxThread == NULL) {
		printf("failed to start rx thread\n");
		return;
	}
