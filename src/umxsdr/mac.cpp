...
BOOLEAN MAC11b_Send (void* args) {
	BOOLEAN bRet =	MAC11a_Send (args);

	//
	// in 11b the carrier sensing state is embeded in rx_state
	//
	current_state = MAC_STATE_RX;
	return bRet;
}
...
BOOLEAN MAC11b_Receive (void*) {
    int moved = pRxSource->Seek(ISource::END_POS);
	while (1) {
		pRxSource->Process ();
		...
		if ( err != E_ERROR_SUCCESS ) {
			...
			if (  err == E_ERROR_FRAME_OK ) {
				...
				//uint i = BB11bDemodCtx.CF_Error::count_barker()+BB11bDemodCtx.CF_Error::count_energy()+BB11bDemodCtx.CF_Error::count_sfd();
			/*
			FILE *fp;
			fp = fopen("C:\\SoraSDK1.7\\src\\umxsdr\\Syncdata\\Wisee_802.11b_three_parts_Count.txt", "a");
			if (fp != NULL)	{
				fprintf(fp,"%d	%d\n", BB11bDemodCtx.CF_Error::count_DC()*4, i);
			}
			fclose(fp);
			*/
